<template>
  <el-form label-position="top" v-if="data" :key="data.key">
    <el-form-item label="字段标识" v-if="data.type !== 'grid'">
      <el-input :disabled="data.options.prohibitDelete" v-model="data.model" />
    </el-form-item>
    ...
    <el-form-item label="选中时的值" v-if="hasKey('activeValue')">
      <el-input v-model="data.options.activeValue" />
    </el-form-item>

    <el-form-item label="非选中时的值" v-if="hasKey('inactiveValue')">
      <el-input v-model="data.options.inactiveValue" />
    </el-form-item>
    ...
    <template v-if="data.type === 'time' || data.type === 'date'">
      <el-form-item label="默认当天">
        <el-switch v-model="data.options.defaultToday" />
      </el-form-item>
    </template>

    <template v-if="data.type === 'time' && !data.options.defaultToday">
    ...
    <template v-if="data.type === 'date' && !data.options.defaultToday">
      <el-form-item label="默认值">
        <el-date-picker
          style="width: 100%"
          v-model="data.options.defaultValue"
    ...
    <template v-if="data.type === 'time' || data.type === 'date'">
      ...
      <el-form-item label="值格式">
        <el-input v-model="data.options.valueFormat" />
      </el-form-item>
    </template>

    <template v-if="data.type === 'imgupload' || data.type === 'fileupload'">
      <el-form-item v-if="data.type === 'imgupload'" label="模式">
      ...
      <el-form-item label="文件大小限制(单位为MB)">
        <el-input-number v-model.number="data.options.size" :min="1" />
      </el-form-item>
    ...
    <template v-if="data.type !== 'grid'">
      <el-form-item
        ...
        v-if="
          hasKey('showinlist') ||
          hasKey('showinapplist') ||
          hasKey('showinsearch') ||
          hasKey('isAbstract') ||
          hasKey('syncSystemField')
        "
      >
        <el-checkbox v-if="hasKey('showinlist')" v-model="data.options.showinlist" >是否在列表显示</el-checkbox>
        <el-checkbox v-if="hasKey('showinapplist')" v-model="data.options.showinapplist">是否显示在app列表</el-checkbox>
        <el-checkbox v-if="hasKey('showinsearch')" v-model="data.options.showinsearch">是否支持搜索</el-checkbox>
        <el-checkbox v-if="hasKey('isAbstract')" v-model="data.options.isAbstract">是否为摘要字段</el-checkbox>
        <el-checkbox v-if="hasKey('syncSystemField')" v-model="data.options.syncSystemField">是否自动读取系统数据</el-checkbox>
      </el-form-item>
      ...
      >
        ...
        <el-checkbox v-if="hasKey('disabled')" v-model="data.options.disabled"
          >禁用</el-checkbox
        >
        <el-checkbox v-if="hasKey('hidden')" v-model="data.options.hidden"
          >隐藏</el-checkbox
        >
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const handleSelectModeChange = (val: boolean) => {
      if (data.value.type === 'imgupload') {
        return
      }
      if (data.value.type === 'fileupload') {
        return
      }
      if (val) {
