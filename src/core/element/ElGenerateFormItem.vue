<template>
  <el-form-item v-if="element && !element.options.hidden" :key="element.key" :label="element.name" :prop="element.model">
    ...
    <template v-if="element.type === 'imgupload'">
      <el-upload
        ...
        :on-remove="handleImgUploadRemove"
        :before-upload="handleImgUploadBefore"
      >
        <SvgIcon v-if="element.options.listType === 'picture-card'" iconClass="insert" />
        <el-button v-else>
          <SvgIcon iconClass="imgupload" style="margin-right: 10px;" />点击上传
        </el-button>
      </el-upload>
    </template>
    ...
    <template v-if="element.type === 'richtexteditor'">
      <RichTextEditor
        v-model:value="data"
...
export default defineComponent({
  ...
  computed: {
    uploadHeaders() {
      let token:any = ''
      if (location.pathname.includes('/implement/')) {
        token = localStorage.getItem('token-implement')
      } else {
  ...
  setup(props) {
    ...
    const handleImgUploadChange = () => {
      hideImgUpload.value = data.value.length >= props.element.options.limit
    }
    const handleImgUploadRemove = (_file: any, fileList: any[]) => {
      data.value = fileList.map(item => {
        return {
          name: item.name,
          url: item.response.data
      ...
      hideImgUpload.value = fileList.length >= props.element.options.limit
    }
    const vaildFileSize = (size: number) => {
      return size < props.element.options.size * 1024 * 1024
    }
    const handleImgUploadBefore = (file: any) => {
      if (props.element.options.size) {
        const fileSizeResult = vaildFileSize(Number(file.size))
        if (!fileSizeResult) {
          ElMessage.error(
            `${file.name}文件过大，文件大小不能超过${props.element.options.size}M`
          )
          return false
        }
      }
...
.img-upload.hide ::v-deep .el-upload--picture-card {
  display: none;
}
.img-upload.hide ::v-deep .el-upload--text {
  display: none;
}
.img-upload ::v-deep .el-upload-list__item-info {
  padding-right: 30px;
}
</style>
