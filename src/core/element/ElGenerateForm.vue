      ...
      for (let index = 0; index < list.length; index++) {
        ...
        if (list[index].type === 'grid') {
          ...
          if (props.value && Object.keys(props.value).includes(model)) {
            ...
            if (list[index].options.defaultValue) {
              state.model[model] = list[index].options.defaultValue
            }
            if ((list[index].type === 'date' || list[index].type === 'time') && list[index].options.defaultToday) {
              state.model[model] = dayjs().format(list[index].options.valueFormat)
            }
          }
          state.rules[model] = list[index].options.rules ? {
            ...list[index].options.rules,
            pattern: list[index].options.rules.pattern ? new RegExp(list[index].options.rules.pattern) : ''
          } : undefined
        }
      }
    }
