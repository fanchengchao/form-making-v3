...
export default defineComponent({
  ...
  props: {
    ...
    advanceFields: {
      type: Array as PropType<Array<string>>,
      default: () => ['imgupload', 'fileupload', 'richtexteditor', 'cascader']
    },
    layoutFields: {
      type: Array as PropType<Array<string>>,
      default: () => ['grid']
  ...
  setup() {
    ...
    const setJson = (json: WidgetForm) => {
      ...
      console.log('state.widgetForm000', state.widgetForm)
      // defaultsDeep(state.widgetForm, json)
      state.widgetForm = JSON.parse(JSON.stringify(json))
      console.log('state.widgetForm', state.widgetForm.config)
      if (json.list.length) {
        state.widgetFormSelect = state.widgetForm.list[0]
      }
    }
